...
var ground = [], airvoid = [], flooringsTiles = [], environment = [];
// platform variables
var platformHeight, platformLength, gapLength;
var platformWidth = 32;
var platformBase = (canvas.height - platformWidth)-60;  // bottom row of the game
...
if (canUseLocalStorage) {
  playSound = (localStorage.getItem('master2077.playSound') === "true")
...
var assetLoader = (function() {
  // images dictionary
  this.imgs        = {
    'bg'            : 'imgs/starrynight.png',
    'sky'           : 'imgs/night_sky2.png',
    'backdrop'      : 'imgs/backcity.png',
    'backdrop2'     : 'imgs/backdrop_ground_night_city2.png',
    'block'         : 'imgs/building.png',
    'avatar_normal' : 'imgs/normal_walk_neon.png',
    'airvoid'         : 'imgs/air.png',
    'block1'        : 'imgs/buildingMid1.png',
    'block2'        : 'imgs/buildingMid2.png',
    'bridge'        : 'imgs/night_bridge.png',
    'MetalObject'         : 'imgs/metal_boxes.png',
    'nightb1'         : 'imgs/nightb1.png',
    'nightb2'         : 'imgs/nightb2.png',
    'cliff'         : 'imgs/CliffRight_night1.png',
    'spikes'        : 'imgs/spikes.png',
    'box'           : 'imgs/special.png',
    'slime'         : 'imgs/slime.png'
  };
  // sounds dictionary
  
  var xsounds = Math.floor((Math.random() * 4) + 1); // get new random number// random music background
  
      if (xsounds == 1) {
        bgsound = "sounds/levelone.mp3";
      }
      else if (xsounds == 2) {
        bgsound = "sounds/levelthree.mp3";
      }
      else if (xsounds == 3) {
        bgsound = "sounds/levelfour.mp3";
      }
      else if (xsounds == 4) {
        bgsound = "sounds/levelseven.mp3";
      }
...
var player = (function(player) {
  ...
  // spritesheets
  player.sheet     = new SpriteSheet('imgs/normal_walk_neon.png', player.width, player.height);
  player.walkAnim  = new Animation(player.sheet, 4, 10, 15);
...
/**
 * Sprites are anything drawn to the screen (ground, flooringsTiles, etc.)
...
function getType() {
  ...
  switch (platformHeight) {
    case 0:
    case 1:
      type = Math.random() > 0.5 ? 'block1' : 'block2';
      break;
    case 2:
      type = 'block';
      break;
...
/**
 * Update all airvoid position and draw.
 */
function updateAirvoid() {
  // animate airvoid
  for (var i = 0; i < airvoid.length; i++) {
    airvoid[i].update();
    airvoid[i].draw();
  }

  // remove airvoid that has gone off screen
  if (airvoid[0] && airvoid[0].x < -platformWidth) {
    var w = airvoid.splice(0, 1)[0];
    w.x = airvoid[airvoid.length-1].x + platformWidth;
    airvoid.push(w);
  }
}
...
/**
 * Update all flooringsTiles position and draw. Also check for collision against the player.
 */
function updateFlooringTilestexture() {
  // animate flooringsTiles
  for (var i = 0; i < flooringsTiles.length; i++) {
    flooringsTiles[i].update();
    flooringsTiles[i].draw();

    // player ran into no floors
    if (player.minDist(flooringsTiles[i]) <= player.width - platformWidth/2) {
      gameOver();
    }
  }

  // remove flooringsTiles that have gone off screen
  if (flooringsTiles[0] && flooringsTiles[0].x < -platformWidth) {
    flooringsTiles.splice(0, 1);
  }
}
...
function spawnSprites() {
  ...
  // then create ground
    ...
    // add random flooringsTiles
    spawnFlooringTilestexture();
  }
  // start over
  else {
...
/**
 * Spawn Flooring (exact jump range check)
 */
function spawnFlooringTilestexture() {
  if (score > 100 && Math.random() > 0.96 && flooringsTiles.length < 3 && platformLength > 5 &&
      (flooringsTiles.length ? canvas.width - flooringsTiles[flooringsTiles.length-1].x >= platformWidth * 3 ||
       canvas.width - flooringsTiles[flooringsTiles.length-1].x < platformWidth : true)) {
  }
}
...
function animate() {
  if (!stop) {
    ...
    // Game Over (remaining sprite images will not be cleared and stops the generating sprite images)
    requestAnimFrame( animate );
    ctx.clearRect(0, 0, canvas.width, canvas.height);
...
/**
 * Start the game - reset all variables and entities, spawn ground and airvoid.
 */
function startGame() {
  document.getElementById('game-over').style.display = 'none';
  document.getElementById('virtualscreen').style.display = 'block';
  document.getElementById('virtualscreen2').style.display = 'none';
  xsounds = Math.floor((Math.random() * 4) + 1);
  ground = [];
  airvoid = [];
  environment = [];
  flooringsTiles = [];
  ...
  for (var i = 0; i < 30; i++) {
    ground.push(new Sprite(i * (platformWidth-3), platformBase - platformHeight * platformSpacer, 'block'));
  }

  for (i = 0; i < canvas.width / 32 + 2; i++) {
    airvoid.push(new Sprite(i * platformWidth, platformBase, 'airvoid'));
  }
...
function gameOver() {
  stop = true;
  document.getElementById('virtualscreen').style.display = 'none';
  document.getElementById('virtualscreen2').style.display = 'block';
  $('#score').html(score);
  $('#game-over').show();
...
$('.sound').click(function() {
  ...
  if (canUseLocalStorage) {
    localStorage.setItem('master2077.playSound', playSound);
  }
